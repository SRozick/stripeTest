...
describe 'PATCH update' do
  ...
  let (:setup)                      {
                                      widget
                                      @before_edit = Widget.new
                                      widget.attributes.each_pair do |key, value|
                                        @before_edit[key] = value
                                      end
                                    }
  let(:attributes_updated_on_form)  { { "name" => 'Great updated product' } }
  let(:attributes_changed_elsewhere){ { "picture" => nil } } #include any attributes changed by controller
  let(:expected_final_attributes)   { attributes_updated_on_form.merge(attributes_changed_elsewhere) }
  let(:action)                      { patch :update, id: widget.id, widget: attributes_updated_on_form }
  ...
  #TODO: Refactor.  Test should not contain expect within let block.  Expect should be within the "It... does" block.
  #TODO: Refactor.  Test should be much easier to read.
  #TODO: The goal of these two tests is to generically validate the patch/put controller action for any controller or set of changed attributes.  Why?  To ensure that if any code changes which unexpectedly changes the model or model object, a test negative results.  <I understand it is not normal practice to test controllers, but I have yet to see a valid explanation.  if TDD and BDD works to develop Ruby, then controllers must be tested too.  And I have seen applications that have grown beyond their acceptable maintenance capacity because they don't have adequate testing at a basic level.>
  ...
  let(:expect_unchanged_to_be_same) {
                                      @before_edit.attributes.each_pair do |key, value|
                                        unchanged = true
                                        ...
                                        if unchanged == true
                                          if @before_edit[key].is_a? ActiveSupport::TimeWithZone
                                            expect(widget[:created_at]).to be_within(0.00001).of(widget.reload[:created_at])
                                          else
                                            expect(@before_edit[key]).to eq(widget.reload[key])
